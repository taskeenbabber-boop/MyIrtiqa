...
export function DebateForm({ onClose }: DebateFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error } = await (supabase as any).from("symposium_debate_submissions").insert({
                name: formData.name,
        ...
        return (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
                ...
                >
                    ...
                    <h2 className="text-2xl font-black text-white uppercase tracking-wide mb-3">Debate Registered!</h2>
                    <p className="text-white/40 text-sm leading-relaxed mb-6">
                        Your registration for the AI Debate has been submitted. We'll notify you at <strong className="text-white/60">{formData.email}</strong> once confirmed.
                    </p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
            ...
            >
                ...
                <div className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4" style={{ borderBottom: `1px solid ${BORDER}` }}>
                    <div>
                        <h2 className="text-xl font-black text-white uppercase tracking-widest">Medical AI Debate</h2>
                        <p className="text-xs text-white/30 mt-1">Defend your stance on controversial topics in Medical AI</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <a
                            href="/guidelines/AI_Debate_Competition_Guidelines.pdf"
                            download
                ...
                <div className="p-6 md:p-8 overflow-y-auto flex-grow scrollbar-hide space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                            { id: "db-name", label: "Full Name", placeholder: "Your full name", value: formData.name, key: "name" as const },
                            { id: "db-email", label: "Email", placeholder: "your@email.com", value: formData.email, key: "email" as const },
                            { id: "db-phone", label: "Phone / WhatsApp", placeholder: "+92 ...", value: formData.phone, key: "phone" as const },
                            { id: "db-inst", label: "Institution", placeholder: "Your institution", value: formData.institution, key: "institution" as const },
                        ].map(f => (
                    ...
                    <div className="rounded-xl p-6" style={{ background: SURFACE, border: `1px solid ${BORDER}` }}>
                        ...
                        >
                            ...
                            <input id="debateReceipt" type="file" required className="hidden"
                                accept="image/*,.pdf"
                ...
                <div className="p-6 flex justify-end" style={{ borderTop: `1px solid ${BORDER}` }}>
                    ...
                    >
                        {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Submitting...</> : <>Enter the Debate <ArrowUpRight className="w-4 h-4" /></>}
                    </button>
                </div>
            </motion.div>
        </div>
