...
const AppContent = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            setShowWhatsApp(scrolled > halfPage);
        };
    ...
    return (
        <>
            ...
            {/* Floating WhatsApp Button — green in light theme, black in dark, scroll-triggered */}
            {!isAdminRoute && (
                <a
                    ...
                    className="fixed bottom-5 right-5 z-50 w-14 h-14 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-500 hover:scale-110 bg-[#25D366] dark:bg-[#1a1a1a]"
                    style={{
                        opacity: showWhatsApp ? 1 : 0,
                        pointerEvents: showWhatsApp ? 'auto' : 'none',
                        transform: showWhatsApp ? 'translateY(0) scale(1)' : 'translateY(20px) scale(0.8)',
                    }}
                >
